...
HellForge is a fork of https://www.darkradiant.net[DarkRadiant], a level editor
for idTech4-based games. While DarkRadiant focuses primarily on The Dark Mod,
HellForge is tailored for editing generic maps targeting
https://github.com/klaussilveira/hellcore[HellCore],
https://github.com/RobertBeckebans/RBDOOM-3-BFG[RBDOOM-3-BFG],
https://github.com/dhewm/dhewm3[dhewm3] and Storm Engine 2. HellForge also
supports Quake 1, Quake 2, Quake 4 and Valve map formats.

This User Guide describes the features and functionality of HellForge, however
...
Doom 3 and similar idTech 4 engines.
...
installed into a directory tree which is accessible both by HellForge (for
editing) and the game engine itself (for playing the mission).

Once assets have been arranged in HellForge according to the wishes of the
...
realtime, making use of the same assets that were accessed by HellForge during
editing.

HellForge also maintains its own separate file containing various editing
...
accompanying `.darkradiant` file can still be edited with HellForge, although
certain HellForge-specific state may be lost.
...
which acts as the parent for new brushes and patches created in HellForge.
...
When running HellForge for the first time, the *Game Setup* dialog will be
shown. This is where you configure the path to the game installation from which
HellForge will load assets, such as textures, models and entity definitions.

image::GameSetupDialog.png[align="center"]

The *Game Setup* dialog contains the following options:

*Game Type*:: HellForge ships with support for several different game engines,
 each of which is contained within a `.game` file. Choose the game type that
 matches the engine you are targeting (Doom 3, Quake 4, The Dark Mod, etc.).

*Engine Path*:: This is the path to the game installation on the system.

*Mod (fs_game)*:: Optional path to a subdirectory containing assets specific to
 a particular mod which is being worked on.

*Mod Base (fs_game_base)*:: This field allows a two-level mod structure, where
 the *Mod Base* can point to a major game mod, while *Mod* can be set to point
...
window without configuring a game installation, in which case HellForge will
show a warning and ask if you wish to proceed. If you do, HellForge will run
but there will be no available textures, models, entities or other game assets.

=== Interface layout

HellForge uses a flexible layout system based on dockable windows which can be moved into
...
HellForge defaults to a dark colour scheme. Several other colour schemes are available and
can be chosen with the *Edit -> Themes...* dialog. The `Maya/Max/Lightwave` and
`Super Mal` themes provide a more neutral, low-contrast look, while the original
`DarkRadiant` scheme provides a traditional black-on-white look.

Colour schemes only affect what is rendered in the 2D and 3D views by HellForge itself.
...
[cols="1h,3"]
|===
|Middle drag|Scroll the view horizontally or vertically
...
be configured, in powers of 2, from 1 up to 256, using the *1-9* keys on the
main keyboard (not the numeric keypad), or the equivalent options in the *Grid*
menu. Smaller grid sizes (0.125, 0.25, 0.5) are available from the *Grid* menu
but have no default key binding.
...
IMPORTANT: The fully lit rendering mode in HellForge is not identical to what
...
shown in the 2D view as a blue diamond. There are various options within the 3D
view itself to adjust the camera position.
...
==== Creating brushes in the 3D view

HellForge allows you to create brushes directly in the 3D camera view, without
needing to switch to the 2D ortho views.

To create a brush, ensure that nothing is selected, then *Left drag* on any
surface in the 3D view. HellForge will detect the surface plane under the
cursor and project the drag onto it, creating a new brush that snaps to the
grid and avoids overlapping with nearby brushes. If no surface is found, the
brush is created on a horizontal plane at the current work zone height.

The following modifiers affect brush creation while dragging:

[cols="1h,3"]
|===
|Shift|Constrain the brush base to a square (equal X and Y dimensions).
|Alt|Switch to height-only mode. Moving the mouse vertically adjusts the brush
height while keeping the base dimensions locked.
|Shift + Alt|Create a perfect cube (equal X, Y and Z dimensions).
|===

==== Resizing brushes with the scroll wheel

When a brush is selected, the scroll wheel in the 3D view can be used to
resize it with different modifiers:

[cols="1h,3"]
|===
|Alt + scroll wheel|Resize the brush face directly under the cursor. Scrolling
moves that face inward or outward along its normal. If the face reaches
minimum thickness, the opposite face is moved instead.
|Ctrl + scroll wheel|Resize both the face under the cursor and the opposite
face symmetrically. The brush grows or shrinks equally on both sides.
|Shift + scroll wheel|Create a new trim brush at the face under the cursor.
The new brush is one grid unit deep and is automatically selected, allowing
subsequent scroll to resize it. Useful for quickly adding trim, frames, or
detail geometry onto existing surfaces.
|===

Without any modifier, the scroll wheel moves the camera forward and backward.
Holding *Shift* without a selection doubles the camera movement speed.

==== Polygon tool in 3D

The polygon tool also works in the 3D viewport. When enabled, you can click
points directly on surfaces in the 3D view to draw polygon shapes.
...
|Left drag (_inside_ object)|Move the selected object(s). Hold *Alt* to
constrain movement to the Z axis only, or *Shift* to constrain to the XY
plane. Hold *Ctrl* to disable grid snapping.
...
|Tab|Cycle forward through items in a selected group
|Shift + Tab|Cycle backward through items in a selected group
|===

TIP: Like other editors in the Radiant family, HellForge offers a rather
...
HellForge provides six buttons to quickly flip or rotate objects (in 90 degree
...
****
Unlike editors that use subtractive geometry (where the world starts as an
infinite solid and rooms are carved out), HellForge uses additive geometry:
the world starts as an infinite void, and all brushes are solid. The play area
must be fully enclosed by solid brushes, otherwise the map will "leak" and fail
to compile.

The exclusive use of solid brushes frees the engine from needing to worry about
...
that way. HellForge offers several options for creating multi-sided brushes in
...
HellForge provides a couple of shortcuts. These options can be found on the
...
This is a legacy tool and generally inferior to *Create Room*.
...
is useful, so it is retained in HellForge.
|===
...
create a doorway or other opening. The *Clipper* tool, accessed with the *C*
...
. Press *C* to activate the Clipper, or click on the respective icon on the
...
disable the Clipper with the *C* key. The Clipper is a toggled tool and will
...
. Select a brush.
. Activate *Select Edges* with the toolbar button or *E* key. HellForge will
...
HellForge provides several Constructive Solid Geometry (CSG) operations in the
*Brush -> CSG* menu for combining or modifying brushes.

[cols="1h,3"]
|===
|Make Hollow|Create a hollow room from the selected brush, with overlapping
wall brushes at the corners. The wall thickness equals the current grid size.
|Make Room|Create a hollow room from the selected brush, without overlapping
walls. The wall thickness equals the current grid size.
|Seal|Create a sealing brush that encloses the bounding box of the current
selection. Useful for quickly sealing a map from the void.
|Make Passable|Turn solid brush volumes into wall shells and carve away interior
geometry to create passable spaces.
|Make Shell|Similar to Passable, but repositions brushes to avoid overlap.
|CSG Subtract (*Shift+U*)|Subtract the selected brush from all brushes it
overlaps. An option to preserve texture on the target brushes is available.
|CSG Merge (*Ctrl+U*)|Merge two or more overlapping brushes into a single
convex brush, if possible.
|CSG Intersect|Compute the intersection of selected brushes, keeping only the
overlapping volume.
|CSG Bridge|Create connecting geometry between two selected faces from different
brushes. Requires exactly two faces to be selected, each with the same number
of vertices.
|===
...
. With the brush selected, choose *Create Simple Patch...* from the *Patch*
menu.
...
Just like with brushes, HellForge offers several default patch shapes beyond
...
3 and HellForge does not permit the creation of a perfect sphere).
...
. Toggle the *Patch Inspector* with *Shift+S* to make the inspector widget
visible.
...
HellForge provides a *Generate* menu with tools for procedurally creating
complex geometry from configurable parameters.

==== Sweep

The Sweep tool (*Generate -> Sweep...*) extrudes one or more selected brushes
along a curve entity (CatmullRom spline or NURBS). Select the brushes to sweep
and a curve entity, then configure the number of segments. The brushes are
extruded along the curve to create curved corridors, pipes, or other swept
shapes.

==== Stairs

The Stairs generator (*Generate -> Stairs...*) creates staircases with several
layout options:

* *Straight stairs*: configurable step count, height, depth, and width.
* *Spiral stairs*: configurable inner/outer radius and total arc angle.
* *L-shaped stairs*: with a landing and a 90-degree turn.
* *U-shaped stairs*: with a landing and a 180-degree turn.

Materials can be assigned to the stair surfaces.

==== Cables

The Cable generator (*Generate -> Cables...*) creates cable and rope geometry
between points. Options include cable presets, gravity simulation, density,
spacing, radius, material, and subdivision control.

==== Arch

The Arch generator (*Generate -> Arch...*) creates curved arch structures with
configurable inner radius, wall thickness, arc angle, starting angle, segment
count, depth, and material.

==== Corner Patch

The Corner Patch generator (*Generate -> Corner Patch...*) creates curved
patch surfaces for smooth corners with configurable arc degrees, radius,
segment resolution, and optional inversion.

==== Building

The Building generator (*Generate -> Building...*) creates complete building
structures from configurable parameters. If a brush is selected, its bounds are
used as the building footprint. Otherwise, the building is created at the
current spawn position.

The dialog provides the following options:

*Dimensions*:: Building width, depth, and overall height.

*Floors*:: Number of floors. Floor height can be set automatically (evenly
divided) or specified manually.

*Walls*:: Wall thickness in map units.

*Windows*:: Window mode (none, regular grid, or custom count), with
configurable width, height, and sill height (distance from floor to window
base).

*Roof*:: Roof type (Flat, Flat with Border, Slanted, or A-Frame), roof height
and border height.

*Columns*:: Optional corner columns with configurable extrusion amount.

*Trim*:: Trim height for edge details, with a separate material assignment.

*Materials*:: Separate material selections for walls and trim surfaces, with
browse buttons to pick from the material library.

==== Tile Map

The Tile Map designer (*Generate -> Tile Map...*) provides a visual grid-based
editor for quickly designing dungeon-style layouts.

The workflow is paint-based: you configure a grid, click to place tiles on a
canvas, then generate the geometry.

*Grid*:: Grid columns and rows (default 16x16), tile width and height (default
128 units), ceiling height per floor, and floor gap spacing between floors.

*Tile types*:: Solid blocks, slope tiles (NW, NE, SW, SE), and curve tiles (NW,
NE, SW, SE). Solid tiles appear green on the canvas, empty tiles in dark gray.
Slopes and curves display as triangles and arcs in the preview.

*Room style*:: Each tile can be generated as Solid (filled brush), Room (hollow
with interior space), Shell (walls only), or Passable (platforms).

*Wall thickness*:: Configurable at 8, 16, 32, or 64 units.

*Multi-floor*:: Each floor maintains its own independent tile layout. Use the
up/down buttons to switch between floors, and add new floors as needed.

*Materials*:: Separate material selections for floor, ceiling, and wall
surfaces.

*Stairs*:: Mark tiles for staircase placement with configurable material, step
count, step height, direction, and an option to build solid from the base.

*Lighting*:: Mark tiles for automatic light placement with configurable radius,
colour (RGB), and light texture.

==== Terrain Generator

The Terrain Generator (*Patch -> Create Terrain...*) creates procedural terrain
patches using noise algorithms. It supports Perlin, Simplex, Fractional Brownian
Motion (FBm) and Ridged Multifractal noise, with configurable seed, scale, and
amplitude parameters.

==== OSM Importer

The OSM importer (*File -> Import OSM...*) is an experimental tool that imports
OpenStreetMap data to generate real-world building and road geometry. Options
include scale control (units per meter), level height, and material assignment
for walls, roofs, floors, roads, and sidewalks.
...
When HellForge first starts up no textures are loaded and this panel is empty.
...
face. HellForge provides the *Surface Inspector* for this purpose, which can
be toggled with the *S* key.
...
order to assist with this, HellForge provides a *shader clipboard* which
...
2. Directly *Alt + Middle click* the face in the 3D view (there is no need to
select it first).
...
HellForge provides certain common functionality to all entities, such as the
...
and patches created inside HellForge, or from an external model file in ASE or
...
view and choose *Create model…*. HellForge will show the model selector
dialog:
...
2D view and choose *Place Player Start here*. HellForge will create the
player start entity at the clicked position.

Since it makes no sense to have more than one player start location, HellForge
will not enable the *Place Player Start here* menu option if there is already
...
When you create a model using the <<CreatingModels,model selector>>, HellForge
...
primitives created within HellForge. In either case, the behaviour of
...
continuously. HellForge offers dedicated commands to convert to/from a
...
*TAB* will cause HellForge to select a different primitive contained within
...
HellForge does not include functionality for compiling a map into the form
needed by the game engine; instead, you must use external tools or the map
compiler built into the game itself.

For Doom 3 and similar idTech 4 engines, the following commands are used within the in-game
...
and HellForge to be visible on the screen as separate windows, which you can
...
After the map compilation has failed with a leak, return to HellForge and
...
The Command Palette (*Ctrl+Shift+P*) provides a quick way to search and execute
any available command in HellForge. Type to filter the list using fuzzy matching.
Each entry shows the command name, description, and keyboard shortcut if one is
assigned.

=== Insert Palette

The Insert Palette (*Ctrl+P*) allows quick entity insertion via a searchable
list. It also includes a goto feature for navigating directly to entities by
name, similar to Sublime Text's goto line number feature.

=== Learner Mode

Learner mode is an onboarding feature for new users that displays keyboard
shortcut hints in the 3D viewport while editing.

=== Game Configuration Manager

The Game Configuration Manager allows managing multiple game setups from the
*File -> Game/Project Setup...* dialog. This is useful when working with
different engines or multiple mod configurations for the same game.

=== Map Format Support

HellForge supports importing and opening maps in several formats beyond the
standard idTech4 `.map` format:

* *Valve VMF*: Source engine map format (*File -> Import Map...*)
* *Quake 1 MAP*: Standard Quake 1 map format
* *Quake 2 MAP*: Standard Quake 2 map format
* *Valve 220 MAP*: Extended MAP format with 220-style texture alignment

=== Model Format Support

In addition to the standard ASE and LWO model formats, HellForge supports:

* *MD5v10*: Standard idTech 4 MD5 model format
* *MD5v11*: Enemy Territory: Quake Wars (ETQW) model format
* *MD5v12*: Storm Engine 2 model format

== Organising a map

Since a fully developed map is large and complex, HellForge provides
...
HellForge allows a number of objects to be grouped together, so that they can
...
HellForge allows a collection of objects to be saved to disk as a *prefab*,
...
prefabs are a HellForge-only feature that have no significance to the engine.
...
Either right-click in the 2D window and choose *Import Prefab...*, or open the
...
HellForge provides a limited ability to export scene geometry into a number of
...
frequently-used map elements — HellForge is not intended to compete with
...
. Select the geometry to be exported. Note that HellForge currently does not
...
To show or hide the *Layers* window, press *Ctrl+L*. The window lists all of
the layers which exist in the current project, and allows you to perform
...
objects, HellForge also provides a mechanism for controlling the visibility of
...
In addition to the filters supplied with the mod distribution, HellForge also
...
become more challenging. To assist with this, HellForge provides an *Entity
List* which functions as an "outline" view of the map, listing all entities and
their child brushes and patches (if any).

The entity list can be accessed by pressing *J*.
...
*Open...*:: Prompt to save and close the current map (if necessary), then
 display a file browser to choose and load an existing map file from disk.

*Open Map from Project...*:: Browse and open a map file from the currently
 configured game project directory.
...
*Merge Map...*:: Open the map merge panel for merging another map into the
 current map with visual diff and conflict resolution.
...
*Import OSM...*:: Open the experimental OpenStreetMap importer to generate map
 geometry from OSM data files.
...
*Export Map...*:: Export the current map to a chosen file format and location.
...
*Export selected as Prefab...*:: Equivalent to *Save selected as Map* but it saves
 the selected items into a <<WorkingWithPrefabs,prefab>> (`*.pfb`) file in the
 standard prefab location.

*Export selected as Model...*:: Export the selected brushes and patches into an
 external model file (ASE, LWO or OBJ format).

*Export selected as Collision Model...*:: Allows a selected map object (typically
 a brush) to be set as the collision model for a particular ASE or LWO model.
 After choosing this option, HellForge will display the model chooser dialog,
...
*Export Region...*:: Save only the objects within the currently-defined region
 to a separate map file.

*Import/Convert Model...*:: Import an external model file and optionally convert
 it between supported formats.

*Reload Declarations / Reload Images / Reload Models / Reload Selected Models*::
 Forces HellForge to re-read the selected category of items from disk,
 refreshing its internal data structures. This is necessary if you have edited
 or re-exported some asset which HellForge is using in the current map and you
...
*Exit*:: Exit HellForge, prompting to save the current map if necessary.
...
clipboard>> to all selected faces with projected coordinates.

*Paste shader (Natural)*:: Paste the shader with natural coordinates, which
flow more naturally over curved surfaces.
...
*Expand selection to all Siblings*:: Select all objects that share the same
 parent entity as the currently-selected primitive.

*Select all of type*:: Select all objects in the map that share the same entity
 class as the currently-selected object.

*Command Palette...*:: Open the command palette (*Ctrl+Shift+P*) for quick
command search and execution.

*Insert Palette...*:: Open the insert palette (*Ctrl+P*) for quick entity
insertion and goto navigation.

*Keyboard Shortcuts...*:: Open the keyboard shortcut editor for viewing and
 customising all keyboard bindings.

*Mouse Bindings...*:: Open the mouse tool mapping dialog for viewing and
 customising mouse button bindings in different view contexts.

*Themes...*:: Open the theme manager for switching between UI colour themes.
...
*Camera -> Center*:: Center the camera on the current selection.

*Camera -> Up Floor / Down Floor*:: Move the camera to the floor above or below.

*Camera -> Far Clip Plane In / Out*:: Move the far clip plane closer to or
 further from the camera.

*Camera -> Next leak spot / Previous leak spot*:: Navigate along a loaded
 pointfile to the next or previous leak location.

*Orthographic -> Next (XY, XZ, YZ)*:: Cycle the current 2D view through the
 three axis orientations.

*Orthographic -> XY (Top) / YZ / XZ*:: Set the 2D view to a specific axis
 orientation.

*Orthographic -> XY 100% / XY Zoom In / XY Zoom Out*:: Reset zoom level or
 zoom the 2D view in or out.

*Show -> Show Angles / Names / Blocks / Coordinates / Window Outline / Axes /
 Workzone / size info*:: Toggle the display of various overlay elements in
 the 2D and 3D views.

*Hide/Show -> Hide Selected*:: Hide the currently selected objects.

*Hide/Show -> Hide Deselected*:: Hide all objects that are not selected.

*Hide/Show -> Show hidden*:: Make all hidden objects visible again.

*Region -> Switch off*:: Disable the current region.

*Region -> Set from XY view*:: Define a region from the visible area of the
 current 2D view.

*Region -> Set from Brush*:: Use the selected brush as the region boundary.

*Region -> Set from Selection*:: Use the bounding box of the current selection
 as the region boundary.

*Background Image...*:: Configure a background image for the 2D view.

=== Modify menu

*Components -> Edges*:: Enter edge editing mode for the selected brush.

*Components -> Vertices*:: Enter vertex editing mode for the selected brush or patch.

*Components -> Faces*:: Enter face editing mode for the selected brush.

*Components -> Entities*:: Enter entity drag mode for selecting and moving entities.

*Nudge -> Left / Right / Up / Down*:: Move the selected object(s) by one grid
 unit in the specified direction.

*Rotate -> X / Y / Z*:: Rotate the selected object(s) by 90 degrees around the
 specified axis.

*Mirror -> X / Y / Z*:: Mirror (flip) the selected object(s) along the
 specified axis.

*Rotate Objects independently*:: Toggle whether each selected object rotates
 around its own center or around the common center of the selection.

*Rotate and scale...*:: Open the transform dialog for entering precise rotation
 and scale values.

*Create Array...*:: Create an array of copies of the selected object(s) with
 configurable count and spacing.

*Scatter...*:: Scatter copies of the selected object(s) over a surface with
 configurable density and randomness.

=== Grid menu

*Snap selected to grid*:: Snap the selected object(s) to the current grid.

*Grid 0.125 .. Grid 256*:: Set the grid size to the specified value. Grid sizes
 1 through 256 are accessible via the *1-9* number keys. Sizes 0.125, 0.25 and
 0.5 are available from the menu only.

=== Map menu

*Find brush...*:: Search for a specific brush by number.

*Find and replace textures...*:: Open a dialog to search for and replace
 textures across the map.

*Map info...*:: Display information about the current map, including counts of
 entities, brushes and patches.

*Skin Editor...*:: Open the skin editor for creating and editing model skin
 definitions.
...
*EntityDef Tree...*:: Show a tree of all available entity types in the current game, along with
...
*MD5 Animation Viewer*:: Open a viewer for previewing MD5 model animations.

*Particle Editor...*:: Open the particle system editor for creating and modifying particle effects.

=== Generate menu

*Sweep...*:: Extrude selected brushes along a curve path.

*Stairs...*:: Generate staircase geometry (straight, spiral, L-shaped, U-shaped).

*Cables...*:: Generate cable/rope geometry between points.

*Arch...*:: Generate curved arch structures.

*Corner Patch...*:: Generate curved corner patch surfaces.

*Building...*:: Generate complete building structures with floors, walls, windows,
and roofing.

*Tile Map...*:: Open the visual tile-based level designer for grid-based map
layout.
...
*CSG -> Seal*:: Create a sealing brush that encloses the bounding box of the current selection.

*CSG -> Make Passable*:: Turn solid volumes into wall shells and carve away interior geometry.

*CSG -> Make Shell*:: Similar to Make Passable but repositions brushes to avoid overlap.

*CSG -> CSG Subtract*:: Subtract the selected brush from overlapping brushes.

*CSG -> CSG Merge*:: Merge overlapping brushes into a single convex brush.

*CSG -> CSG Intersect*:: Keep only the overlapping volume of selected brushes.

*CSG -> CSG Bridge*:: Create connecting geometry between two selected faces.
...
*Polygon Mode*:: Toggle the polygon drawing tool for creating arbitrary polygon shapes.

*Texture lock*:: Toggle whether texture alignment is preserved when brushes are moved or resized.

*Create Decal Patches*:: Create decal patches on the selected brush faces.

*Make Visportal*:: Convert the selected brush into a visportal for controlling rendering visibility.

*Make Detail / Make Structural*:: These options exist to support legacy games with detail/structural
brush classification.

=== Patch menu

*Create Simple Patch...*:: Create a flat rectangular patch from the selected brush.

*Create Terrain...*:: Open the terrain generator to create procedural terrain patches.

*Create End cap / Create Bevel*:: Create arch-shaped patch surfaces.

*Create Cone / Create Cylinder / Create Sphere*:: Create standard patch shapes.

*More cylinders -> Dense Cylinder / Very Dense Cylinder / Square Cylinder*::
 Create cylinder variants with different control point densities.

*Insert -> Insert 2 columns/rows at beginning/end*:: Add control points without
 changing patch dimensions.

*Extend -> Extend by 2 columns/rows at beginning/end*:: Add control points while
 extending the patch.

*Delete -> Delete 2 columns/rows from beginning/end*:: Remove control points
 without changing patch dimensions.

*Matrix -> Invert*:: Invert the curve direction.

*Matrix -> Re-disperse -> Rows / Columns*:: Redistribute control points evenly.

*Matrix -> Transpose*:: Swap the row and column dimensions of the patch.

*Thicken...*:: Extrude the patch to give it thickness.

*Cap...*:: Cap the open ends of a patch to create a closed surface.

*Stitch Patch Textures*:: Align textures across adjacent patches.

*Bulge Patch...*:: Apply a bulge deformation to the selected patch.

*Weld/Merge Patches*:: Weld or merge two selected patches into one.

=== Curve menu

*Create NURBS Curve*:: Create a NURBS spline curve entity.

*Create CatmullRom Curve*:: Create a Catmull-Rom spline curve entity.

*Convert NURBS <-> CatmullRom*:: Convert a curve between NURBS and Catmull-Rom
 representations.

*Append Curve Control Point*:: Add a new control point at the end of the curve.

*Insert Curve Control Points*:: Insert new control points between existing ones.

*Remove Curve Control Points*:: Remove control points from the curve.

=== Help menu

*About*:: Display version and build information about HellForge.

*Community Forum*:: Open the community forum in a web browser.

*Script Reference Wiki Article*:: Open the scripting reference documentation
 in a web browser.

*User Guide (Online)*:: Open the online version of this user guide.

*User Guide (Local)*:: Open the local copy of this user guide.

== MCP Server

HellForge includes an MCP (Model Context Protocol) server that allows AI agents
to operate the editor programmatically. This enables AI-assisted level design
workflows where an agent can build and modify maps using natural language
instructions.

=== Architecture

The MCP system consists of two components:

* *HellForge MCP Plugin*: A C++ plugin loaded by HellForge that starts a TCP
  server on `localhost:13646` and exposes editor operations as JSON-RPC methods.
  All operations are dispatched to the main wxWidgets thread for thread safety.
* *Python MCP Server*: Bridges the MCP protocol to the HellForge plugin. This
  is what AI tools like Claude Code connect to.

=== Setup

==== 1. Configure your AI tool

Add the MCP server to your tool's configuration. For Claude Code, add to
...
The `run.sh` wrapper handles the working directory and runs the server using
https://docs.astral.sh/uv/[uv]. Alternatively, install and run the package
directly:
...
Start HellForge normally. The MCP plugin will begin listening on port 13646.
The Python server connects to it automatically when a tool is called.

==== 3. Verify

In Claude Code, run `/mcp` to check the server is connected. The `hellforge`
server should be listed with 75+ tools.

=== Capabilities

The MCP server exposes tools for:

* Creating and manipulating brushes and entities
* Browsing and applying materials
* Creating particles in the particle editor
* Exploring and querying existing maps
* Capturing camera views for visual feedback
* Saving maps

=== Example

The following sequence of tool calls creates a simple sealed room with a light
and player start:

----
get_game_config()
list_materials(filter="floor", limit=10)
create_brush(min=[-128,-128,0], max=[128,128,8],
     material="textures/common/caulk")
create_brush(min=[-128,-128,120], max=[128,128,128],
     material="textures/common/caulk")
create_brush(min=[-128,-128,0], max=[-120,128,120],
...
*MCP server not showing*: Check that `run.sh` is executable and that `uv` is
installed and on PATH. Try running `uv run hellforge_mcp.py` manually from
`tools/mcp-server/` to see errors.

*Tools return "Connection error"*: Make sure HellForge is running with the MCP
plugin loaded. Check the HellForge console for `[MCP] Listening on
localhost:13646`. Verify nothing else is using port 13646.

*`list_materials` returns empty*: HellForge must have loaded a game
configuration with `.mtr` files. Make sure a game is configured and the VFS
paths are set.

== Configuring HellForge

HellForge offers a large number of configurable options which can be used to
...
*Higher selection priority for entities*:: When using the mouse to click on and
 ...
 controls whether HellForge will prefer to select the entity (enabled) or the
 brush (disabled).

==== User Interface settings

*Start on Monitor*:: If you have more than one monitor, you can choose which
monitor HellForge will start on using this combo box.
...
you to choose which language is used for the HellForge interface.

==== Autosave settings

*Enable Autosave*:: Controls whether the autosave feature is active or disabled.

*Autosave Interval*:: Use this slider to choose how often HellForge will make
 an autosave. By default, autosaves are made every 5 minutes.

*Save Snapshots*:: If this option is enabled, HellForge will make each
 ...
 against HellForge or system crashes but not against long-term erroneous
 changes.
...
*Max Snapshot size per map*:: This allows you to control the amount of disk
 ...
 grows above this limit, HellForge will start to delete old snapshots.
...
*Ignore light volume bounds when calculating default rotation pivot location*::
 ...
 when multiple objects are selected. HellForge will place the rotation widget
...
*Undo Queue Size*:: Use this spinbox to control the number of operations which
 are saved onto HellForge's internal undo stack. Larger values provide a
...
*Open last map on startup*:: Enable this option to make HellForge
...
HellForge is designed to work out of the box with a standard Doom 3 or Dark
Mod game installation, however it also offers a few features aimed at developers
of the game itself.

=== Controlling asset visibility

As the development of a game progresses, certain assets may become obsolete or
deprecated, either because they are no longer needed or because they fall short
of the quality standard that is expected of new assets. If these assets have
already been distributed, removing them from the game installation may be
problematic, as maps which depend on them will fail to load correctly.

HellForge offers a mechanism to deal with this by allowing certain assets to
...
The mechanism for hiding assets depends on the asset type: models, materials, particles
and sound shaders use a file-based `assets.lst` approach, whereas entity classes use a
dedicated spawnarg.

==== Hiding models, materials, particles or sound shaders

To change the visibility of these types of asset, you need to create a file called
`assets.lst` which resides in the top-level `models`, `materials`, `particles` or `sound`
directory in the mod installation. This file may be inside a PK4 or in an extracted
directory tree.
...
.assets.lst
----
models/furniture/old_chair.lwo=hidden
models/props/broken_table.ase=hidden
----

Asset paths listed on the left-hand side are interpreted relative to the
directory containing `assets.lst`. The visibility value on the right-hand side
must be either `hidden` or `normal`; a value of `normal` is equivalent to not
listing the file at all, and results in no change to its visibility.

NOTE: Only *files* may be listed in an `assets.lst`, not items in a virtual hierarchy such
as the material shader tree. When a `.mtr` or `.sndshd` file is listed as hidden, all
shaders contained within it will be hidden in HellForge. This means that in order to
hide specific shaders, you must move these into one or more source files which are listed
as hidden.

==== Hiding entities

To hide an entity class, edit the `.def` file and set a "editor_visibility"
spawnarg on the entity class, giving it the value "hidden". This will cause the
entity class to be hidden from the Create Entity dialog and any other entity
class chooser.

The visibility spawnarg is not inherited, which makes it possible to hide a base
class (perhaps because it should not be created directly), without affecting the
visibility of entity classes which inherit from it.

== Keyboard shortcut reference

The following table lists the default keyboard shortcuts in HellForge. These
can be customized via *Edit -> Keyboard Shortcuts...*.
...
|Ctrl+X|Cut
|Ctrl+V|Paste
|Alt+V|Paste to camera
|Space|Clone (duplicate) selection
|Backspace|Delete selection
|Escape|Deselect all
|P|Preferences
|O|Toggle console
|Ctrl+Shift+P|Command Palette
|Ctrl+P|Insert Palette
|===

=== Camera movement

These keys are active when the 3D camera view has focus. Note that *W*, *A*,
*S*, *D* and *X* also have non-camera bindings (Surface Inspector, etc.) that
apply when focus is elsewhere.
...
|Q|Move up
|X|Move down
|KP -|Decrease camera speed
|KP +|Increase camera speed
|End|Center view on selection
|===
...
|===
|I|Invert selection
|Shift+A|Select all of type
|Ctrl+Alt+E|Expand selection to siblings
|Ctrl+Alt+Shift+E|Select parent entities
|Tab|Cycle forward in group
|Shift+Tab|Cycle backward in group
|Shift+G|Revert to worldspawn
|Ctrl+Shift+G|Group selected
|Ctrl+Shift+U|Ungroup selected
|Ctrl+K|Connect selected entities
|===
...
|R|Rotate mode
|G|Resize/drag mode
|V|Vertex editing mode
|E|Edge editing mode
|F|Face editing mode
|Ctrl+F|Toggle selection focus
|Ctrl+R|Toggle rotation pivot
|Ctrl+G|Snap to grid
|===
...
|Ctrl+T|Toggle Texture Browser
|Ctrl+Alt+T|Toggle Texture Tool
|Ctrl+Alt+Shift+T|Toggle Texture Tool (main control)
|Ctrl+N|Natural texture alignment
|Shift+N|Normalize texture
|Shift+Arrow keys|Shift texture
|Ctrl+Arrow keys|Scale texture
|Shift+PageDown|Rotate texture clockwise
|Alt+PageUp|Rotate texture counter-clockwise
|Shift++|Texture tool grid up
|Shift+-|Texture tool grid down
|Alt+M|Texture tool merge items
|===
...
|Shift+Enter|Split selection
|Ctrl+Enter|Flip clip orientation
|Shift+U|CSG Subtract
|Ctrl+U|CSG Merge
|Ctrl+3..9|Create 3..9 sided brush
|Ctrl+M|Make detail
|Ctrl+Shift+S|Make structural
|Ctrl+Shift+R|Set region from selection
|===
...
[cols="1h,3"]
|===
|Shift+P|Create simple patch
|Shift+S|Toggle Patch Inspector
|Ctrl+I|Invert curve
|Ctrl+E|Redisperse rows
|Ctrl+Shift+E|Redisperse columns
|Ctrl+Shift+M|Transpose matrix
|Shift+C|Cap selection
|Ctrl+KP +|Insert row at end
|Ctrl+Shift+KP +|Insert column at end
|Ctrl+KP -|Delete row at end
|Ctrl+Shift+KP -|Delete column at end
|===
...
[cols="1h,3"]
|===
|Ctrl+Tab|Next ortho view (XY, XZ, YZ)
|Ctrl+Shift+Tab|Center all XY views on camera
|Ctrl+Alt+Tab|Center XY view on camera
|Shift+Ctrl+V|Toggle view layout
|Ctrl+Shift+C|Toggle camera panel
|Ctrl+Alt+N|New ortho view
|Shift+X|Toggle crosshairs
|0|Toggle grid display
|1|Set grid 1
|2|Set grid 2
|3|Set grid 4
|4|Set grid 8
...
|8|Set grid 128
|9|Set grid 256
|-|Grid size up (larger spacing)
|+|Grid size down (smaller spacing)
|F3|Toggle preview mode
|===
...
|PageUp|Up floor
|PageDown|Down floor
|KP /|Far clip plane in
|KP *|Far clip plane out
|Shift+F|Toggle far clip plane
|Ctrl+Shift+K|Next leak spot
|Ctrl+Shift+L|Previous leak spot
|Delete|Zoom in (2D view)
|Insert|Zoom out (2D view)
|===
...
[cols="1h,3"]
|===
|Alt+Arrow keys|Nudge selection in the pressed direction
|Shift+KP +|Move selection up
|Shift+KP -|Move selection down
|===
...
|L|Light Inspector
|M|Media Browser
|Ctrl+Shift+A|AAS Visualization panel
|Ctrl+Shift+Alt+F|Show all light radii
|Alt+O|Background image panel
|===
...
[cols="1h,3"]
|===
|Ctrl+Alt+1..0|Save camera position 1-10
|Alt+1..0|Load camera position 1-10
|===

=== Mouse bindings (3D camera view)

[cols="1h,3"]
|===
|RMB|Free camera mode
|Alt+RMB|Jump to object under cursor
|MMB|Pan view
|Alt+MMB|Pick shader from surface
|Ctrl+MMB|Paste shader (projected)
|Shift+MMB|Paste shader (Natural)
|Alt+Ctrl+MMB|Paste shader to entire brush
|Alt+Shift+MMB|Paste shader name only
|Ctrl+Shift+MMB|Decal shooter
|LMB|Manipulate / create brush (context dependent)
|Shift+LMB|Select/deselect
|Ctrl+Shift+LMB|Select face only
|Alt+Shift+LMB|Cycle selection
|Ctrl+Shift+Alt+LMB|Cycle selection (faces only)
|===

=== Mouse bindings (2D ortho view)

[cols="1h,3"]
|===
|LMB|Manipulate / create brush / clip / polygon (context dependent)
|Shift+LMB|Select/deselect
|Alt+Shift+LMB|Cycle selection
|MMB|Scroll view
|Shift+RMB|Zoom view
|Ctrl+MMB|Measurement tool
|===

=== Mouse bindings (Texture Tool)

[cols="1h,3"]
|===
|LMB|Manipulate texture
|Shift+LMB|Select/deselect
|Alt+Shift+LMB|Cycle selection
|RMB|Scroll view
|===